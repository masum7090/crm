<x-admin-layout>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 mt-6 max-w-4xl mx-auto">
        ...
        <form action="{{ route('admin.orders.update', $order->id) }}" method="POST">
                    ...
                    <div class="item-row border p-4 rounded-lg mb-4 bg-gray-50 relative group">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                            ...
                            <div class="md:col-span-3">
                                ...
                                <input type="number" name="items[{{ $index }}][quantity]" value="{{ $item->quantity }}" min="1" class="w-full border-gray-300 rounded-lg text-sm qty-input" oninput="calculateTotal()">
                            </div>
            ...
            <div class="bg-blue-50 border-blue-100 border p-4 rounded-xl flex justify-between items-center mb-6">
                <span class="font-bold text-blue-800">Total Order Amount:</span>
                <span id="grand-total" class="text-xl font-black text-blue-600">${{ number_format($order->total_amount, 2) }}</span>
            </div>

            <div class="flex justify-end gap-3 mt-8">
                <a href="{{ route('admin.orders.show', $order->id) }}" class="px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">Cancel</a>
                <button type="submit" class="px-6 py-2.5 text-sm font-bold text-white bg-blue-600 rounded-lg hover:bg-blue-700 shadow-md transform transition active:scale-95">Update Order</button>
            </div>
        </form>
    </div>
    ...
    <script>
        ...
        function addItem() {
            ...
            const descInput = template.querySelector('input[name*="[description]"]');
            if (descInput) descInput.value = "";
            template.querySelector('.qty-input').value = 1;
            template.querySelector('.price-input').value = "0.00";
            
            // Update names - use regex to catch any index
            template.querySelectorAll('[name*="items["]').forEach(input => {
